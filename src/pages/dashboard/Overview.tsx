  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Good morning{brand.name ? `, ${brand.name}` : ''}</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>Here's your marketing performance summary</p>
        </div>
      ...
      <div className="grid grid-cols-[1fr_380px] gap-6">
        ...
        <Card>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Morning Briefing</h3>
            <span className="text-xs px-2 py-0.5 rounded-full ml-auto" style={{ background: 'var(--accent-light)', color: 'var(--accent-dark)' }}>Today</span>
          </div>
          <div className="rounded-lg p-4 mb-4" style={{ background: 'var(--bg-alt)', border: '1px solid var(--border-light)' }}>
            <p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              <strong style={{ color: 'var(--text-primary)' }}>Highlights:</strong> Your blog post "10 Ways to Boost Retention" drove <strong style={{ color: '#00D4AA' }}>2,400 organic visits</strong> yesterday — 3x your daily average. The retargeting campaign is your top performer at <strong style={{ color: '#00D4AA' }}>5.1x ROAS</strong>.
            </p>
          </div>
          <div className="rounded-lg p-4 mb-4" style={{ background: 'var(--bg-alt)', border: '1px solid var(--border-light)' }}>
            <p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              <strong style={{ color: 'var(--text-primary)' }}>Actions taken:</strong> I paused the underperforming "Summer Sale" ad set and reallocated budget. Scheduled 4 amplification posts. Sent the weekly newsletter (42% open rate, 8.3% click rate — both above benchmark).
            </p>
          </div>
          <div className="rounded-lg p-4" style={{ background: 'rgba(124,92,252,0.06)', border: '1px solid rgba(124,92,252,0.12)' }}>
            <p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              <strong style={{ color: 'var(--accent-dark)' }}>Recommendations:</strong> Consider doubling the retargeting budget — it's consistently outperforming. Your "pricing page" keyword is rising to position #4 — one more optimised post could push it to #1. Competitor X just launched a TikTok campaign; I've drafted a response strategy.
            </p>
          </div>
        </Card>
        ...
        {/* Activity Feed */}
        <Card>
          <h3 className="text-sm font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>Today's Activity</h3>
          <div className="space-y-3">
            {recentActions.map((a, i) => (
              <div key={i} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: 'var(--bg-alt)' }}>
                    <a.icon size={12} style={{ color: 'var(--accent-dark)' }} />
                  </div>
                  {i < recentActions.length - 1 && <div className="w-px flex-1 mt-1" style={{ background: 'var(--border-light)' }} />}
                </div>
                <div className="pb-3">
                  <p className="text-xs mb-0.5" style={{ color: 'var(--text-tertiary)' }}>{a.time}</p>
                  <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{a.action}</p>
                </div>
              </div>
            ))}
          </div>
