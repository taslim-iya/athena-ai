  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'var(--text)' }}>Integrations</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--text-2)' }}>{connected} of {integrations.length} connected</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {categories.map(cat => (
          <div key={cat}>
            <h3 className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: 'var(--text-3)' }}>{CATEGORY_LABELS[cat] || cat}</h3>
              ...
              {integrations.filter(i => i.category === cat).map(integ => (
                <div key={integ.id} className="rounded-xl p-4 flex items-center justify-between transition-all hover:scale-[1.01]" style={{
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium" style={{ color: 'var(--text)' }}>{integ.name}</p>
                      {integ.connected && integ.lastSync && (
                        <p className="text-xs" style={{ color: 'var(--text-3)' }}>Synced {integ.lastSync}</p>
                      )}
                    </div>
                  </div>
                  <button onClick={() => toggleIntegration(integ.id)} className="px-3 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1 transition" style={{
                    background: integ.connected ? 'rgba(0,212,170,0.1)' : 'var(--surface-3)',
                    color: integ.connected ? '#00D4AA' : 'var(--text-2)',
