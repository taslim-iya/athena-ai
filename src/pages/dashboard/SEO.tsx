  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'var(--text)' }}>SEO & Content</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--text-2)' }}>Keyword rankings, blog posts, and content optimisation</p>
        </div>
      </div>
      ...
      <Card className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Generate Blog Post</h3>
        </div>
        <div className="flex gap-3">
          <input value={topic} onChange={e => setTopic(e.target.value)} onKeyDown={e => e.key === 'Enter' && generatePost()} placeholder="Enter a topic (e.g. 'How to reduce churn by 50%')" className="flex-1 rounded-lg px-3 py-2.5 text-sm border" style={{ background: 'var(--bg)', borderColor: 'var(--border)', color: 'var(--text)' }} />
      ...
      <div className="grid grid-cols-[1fr_400px] gap-6">
        {/* Keyword Rankings */}
        <Card>
          <h3 className="text-sm font-semibold mb-4" style={{ color: 'var(--text)' }}>Keyword Rankings</h3>
          <div className="space-y-0">
            <div className="grid grid-cols-[1fr_70px_70px_80px_80px] gap-2 pb-2 mb-2" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
              {['Keyword', 'Position', 'Change', 'Volume', 'Difficulty'].map(h => (
                <span key={h} className="text-xs font-medium" style={{ color: 'var(--text-3)' }}>{h}</span>
              ))}
            </div>
            {rankings.map(r => (
              <div key={r.keyword} className="grid grid-cols-[1fr_70px_70px_80px_80px] gap-2 py-2.5" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                <span className="text-sm" style={{ color: 'var(--text)' }}>{r.keyword}</span>
                <span className="text-sm font-medium" style={{ color: r.position <= 3 ? '#00D4AA' : r.position <= 10 ? '#FEBC2E' : 'var(--text-2)' }}>#{r.position}</span>
                <span className="text-xs flex items-center gap-0.5" style={{ color: r.change > 0 ? '#00D4AA' : r.change < 0 ? '#FF6B6B' : 'var(--text-3)' }}>
                  {r.change > 0 ? <ArrowUp size={10} /> : r.change < 0 ? <ArrowDown size={10} /> : <Minus size={10} />}
                  {Math.abs(r.change)}
                </span>
                <span className="text-sm" style={{ color: 'var(--text-2)' }}>{r.volume.toLocaleString()}</span>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs" style={{ color: 'var(--text-3)' }}>{r.difficulty}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Recent Posts */}
        <Card>
          <h3 className="text-sm font-semibold mb-4" style={{ color: 'var(--text)' }}>Blog Posts</h3>
          {blogPosts.length === 0 ? (
            <div className="text-center py-8">
              <FileText size={24} className="mx-auto mb-2" style={{ color: 'var(--text-3)', opacity: 0.4 }} />
              <p className="text-sm" style={{ color: 'var(--text-3)' }}>No posts yet. Generate your first one above.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {blogPosts.slice(0, 8).map(p => (
                <div key={p.id} className="rounded-lg p-3" style={{ background: 'var(--surface-2)', border: '1px solid var(--border-subtle)' }}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium truncate" style={{ color: 'var(--text)' }}>{p.title}</span>
                    <span className="text-xs px-2 py-0.5 rounded-full flex-shrink-0" style={{ background: p.status === 'published' ? 'rgba(0,212,170,0.1)' : 'var(--surface-3)', color: p.status === 'published' ? '#00D4AA' : 'var(--text-3)' }}>
                      {p.status}
                    </span>
                  </div>
